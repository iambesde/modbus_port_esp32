...
//    MB_LOG(TAG, "Create socket successful" );
    FD_ZERO( &allset );
...
//	MB_LOG(TAG, "xMBTCPPortGetRequest" );
...
//	MB_LOG(TAG, "xMBTCPPortSendResponse" );
    do
    {
...
//	MB_LOG( TAG, "prvbMBPortAcceptClient");

    if( xClientSocket != INVALID_SOCKET )
    {
//        MB_LOG( TAG, "can't accept new client. all connections in use." );
        bOkay = FALSE;
    }
